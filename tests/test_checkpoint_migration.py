...
# prior that must widen to the event / Neow layout. Fixed history.
...
def test_shipped_4949_prior_widens_to_shop_boss_layout() -> None:
    ...
    assert SHIPPED_0_7_0_FEATURE_DIM in _KNOWN_PRIOR_FEATURE_DIMS
    target = _current_feature_dim()
    # The shop / boss-relic blocks were the first append after 4949 (locked exactly by
    # SHIPPED_0_8_0_FEATURE_DIM below); the later event / Neow blocks add more, so the
    # current width is at least that much wider.
...
def test_shipped_5571_prior_widens_to_event_neow_layout() -> None:
    ...
    before the event / Neow blocks) must widen to the current layout: its trained columns
    preserved as the leading prefix, the appended event one-hot and Neow bonus / drawback
    columns zero-initialized. The 0.8.0 width is locked symbolically as 4949 + the shop /
    boss-relic append.
    """
